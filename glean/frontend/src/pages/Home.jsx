...
const Home = () => {
  ...
  const fetchDocumentSummary = async (docId) => {
    try {
      ...
      // Get document extraction from backend
  ...
  const handleSelectDoc = (doc) => {
    setSelectedDoc(doc);
    setPdfUrl(`${BACKEND_URL}/doc/${doc.id}/view`); // Use local backend
  ...
  return (
    <div className="flex h-screen bg-gradient-to-b from-[#1F2430] to-[#12171D] text-slate-100">
      ...
      <Chatbot activeCase={selectedDoc?.id} />
    </div>
  );
};
